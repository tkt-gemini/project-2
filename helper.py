...
class Filter:
    ...
    def __first_person_ratio(self, text: str) -> float:
        ...
        fp_count = sum(
            1 for sent in sentences for w in sent.split() if w in FIRST_PERSON_PRONOUNS
        )
    ...
    def noise(
        ...
        author = (
            X["author"].fillna("")
            if "author" in X.columns
            else pd.Series([""] * n_orig)
        )

        m_del = (
            post.str.strip().str.lower().isin({"[removed]", "[deleted]", "", "nan"})
            | post.isna()
        )
        ...
        for reason, count in stats.items():
            print(
                f"  Convert to NaN ({reason}): {count:,} ({count / n_orig * 100:.1f}%)"
            )
    ...
    def clinical(
        ...
        meta_path = checkpoint_dir / "_meta.json"
        if meta_path.exists():
            with open(meta_path, "r") as fh:
                saved_meta = json.load(fh)
            if saved_meta.get("n_texts") != n_orig:
                print(
                    f"Dataset size changed "
                    f"({saved_meta['n_texts']} → {n_orig}). Clearing checkpoints..."
                )
        ...
        ckpts = sorted(
            checkpoint_dir.glob("batch_*.pkl"), key=lambda p: int(p.stem.split("_")[1])
        )
        last_done = int(ckpts[-1].stem.split("_")[1]) if ckpts else -1

        if last_done >= n_batches:
            print(
                f"Stale checkpoints detected (last={last_done}, expected<{n_batches}) – clearing..."
            )
            for _ckpt in checkpoint_dir.glob("batch_*.pkl"):
                _ckpt.unlink()
            if meta_path.exists():
        ...
        # FIX: Use actual DataFrame index values as record keys so that
        # result_df.reindex(X_out.index) aligns correctly regardless of whether
        # X has a default RangeIndex or a custom index (e.g. after partial
        # reset_index calls).
        actual_indices = list(X.index)
        ...
        if last_done >= 0:
            ...
            for p in tqdm(
                sorted(checkpoint_dir.glob("batch_*.pkl")),
                desc="  Load checkpoints",
                unit="batch",
            ):
                ...
                for _, drop, masked, _ in batch:
                    if drop:
                        drop_no_fp += 1
                    elif pd.notna(masked):
                        kept_fp_high += 1
                    else:
                        kept_pattern += 1
                all_results.extend(batch)
        else:
            # Write metadata only on a fresh run so the count is always valid.
            with open(meta_path, "w") as fh:
                json.dump({"n_texts": n_orig}, fh)

        print(f"\n[CLINICAL FILTER - MASK] {n_orig:,} posts | device: CPU")

        pbar = tqdm(
            range(last_done + 1, n_batches),
            desc="Batch",
            unit="batch",
            dynamic_ncols=True,
        )
        for batch_i in pbar:
            ...
            for i, raw in enumerate(texts[start:end]):
                real_idx = actual_indices[start + i]  # actual DataFrame index key
                ...
                if fp >= fp_threshold:
                    ...
                    batch_results.append(
                        (real_idx, False, self.__masking(cleaned), cleaned)
                    )
                elif not self.__is_self_disclosure(cleaned):
                    drop_no_fp += 1
                    batch_results.append((real_idx, True, np.nan, np.nan))
                else:
                    kept_pattern += 1
                    batch_results.append(
                        (real_idx, False, self.__masking(cleaned), cleaned)
            ...
            with open(checkpoint_dir / f"batch_{batch_i}.pkl", "wb") as fh:
        ...
        print(f"Total kept:        {n_kept:,} ({n_kept / n_orig * 100:.1f}%)")
...
class NRCLexicon:
    ...
    def __get_emo(self, path: str):
        ...
        ).reindex(columns=self.EMOTION, fill_value=0)
    ...
    def getlist(self, text: str) -> list[float]:
        ...
        _expected_len = len(self.EMOTION) + len(self.VAD) * 3  # 10 + 9 = 19

        if len(words) == 0:
            result = np.zeros(_expected_len).tolist()
            assert len(result) == _expected_len, (
                f"Zero-vector length mismatch: {len(result)}"
            )
            return result

        emo = np.array(
            [self.__lex["emo"].get(w, self.__emo_zero.copy()) for w in words],
            dtype=float,
        )
        vad = np.array(
            [self.__lex["vad"].get(w, self.__vad_zero.copy()) for w in words],
            dtype=float,
        )

        result = np.concatenate(
            [emo.std(0), vad.mean(0), vad.max(0), vad.std(0)]
        ).tolist()
        assert len(result) == _expected_len, (
            f"NRC feature length mismatch: expected {_expected_len}, got {len(result)}"
        )
        return result
...
class Psychological(BaseEstimator, TransformerMixin):
    ...
    # FIX: Exclude the large spaCy model from pickle so that saved feature
    # engineers (and inference pipelines) are significantly smaller on disk.
    # The model is re-loaded transparently when the object is unpickled.
    ...
    def fnames(self) -> list[str]:
        ...
        nrc_names = [f"nrc_emo_{e}_std" for e in self.nrc.EMOTION] + [
            f"nrc_vad_{v}_{s}" for v in self.nrc.VAD for s in ["mean", "max", "std"]
        ]
        return (
            [f"{p.lower()}_ratio" for p in self.__LINGUI]
            + self.__VADER
            + empath_names
            + nrc_names
        )
